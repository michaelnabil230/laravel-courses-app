                                ...
                                <form action="{{ route('dashboard.admins.update', $admin->id) }}" method="post">
                                    ...
                                    <div class="form-group">
                                        <div class="card card-primary card-tabs">
                                            @php($models = config('permission.models_permissions'))
                                            <style>
                                            ...
                                            <div class="card-header p-0 pt-1">
                                                <ul class="nav nav-tabs" id="custom-tabs-two-tab" role="tablist">
                                                    ...
                                                    @foreach ($models as $model => $permissions)
                                                        <li class="nav-item">
                                                            <a class="nav-link {{ $loop->first == 0 ? 'active' : '' }}"
                                                                id="{{ $model }}-tab" data-toggle="pill"
                                                                href="#{{ $model }}" role="tab"
                                                                aria-controls="{{ $model }}"
                                                                aria-selected="true">{{ $model }}
                                                            </a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            ...
                                            <div class="card-body">
                                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                                    @foreach ($models as $model => $permissions)
                                                        <div class="tab-pane fade {{ $loop->first == 0 ? 'show active' : '' }}"
                                                            id="{{ $model }}" role="tabpanel"
                                                            aria-labelledby="{{ $model }}-tab">
                                                            @foreach ($permissions as $permission)
                                                                <label>
                                                                    <input type="checkbox" name="permissions[]"
                                                                        {{ $admin->hasPermissionTo($permission . '_' . $model) ? 'checked' : '' }}
